...
export class LinakageTableClass {
  ...
  public addTermsToSetSQL = async (setId: number, termIds: number[]) => {
    try {
      ...
      const existingIds = (existingRows as any[]).map(
        (row) => row.word_id
      );
  ...
  public removeTermsFromSetSQL = async (setId: number, termIds: number[]) => {
    try {
      ...
      const missingIds = termIds.filter(
  ...
  public getTermsForSetSQL = async (setId: number, language: string) => {
    ...
    try {
      const query = `
        SELECT 
          w.*, 
          l.primaryid AS primaryIDLinkage
        FROM ${wordTableName} w
        JOIN ${this.tableLinkageName} l 
          ON w.primaryid = l.word_id
        WHERE l.set_id = ?
